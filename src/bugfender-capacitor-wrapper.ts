...
import {
  formatLogEntryText,
  RegisterBrowserEventsHandler,
  RegisterUIEventsHandler,
  format,
  LogLevel,
  PrintToConsole,
...
export class BugfenderCapacitorWrapper implements BugfenderFacade {
  ...
  getDeviceURL(): Promise<string> {
    return this.bugfenderCapacitor.getDeviceURL().then(this.urlToString);
  }

  getSessionURL(): Promise<string> {
    return this.bugfenderCapacitor.getSessionURL().then(this.urlToString);
  }
  ...
  getUserFeedback(options?: UserFeedbackOptions): Promise<UserFeedbackResult> {
    ...
    return new Promise<UserFeedbackResult>((resolve) =>
      this.bugfenderCapacitor
        .getUserFeedback(sanitizedOptions)
        .then((response) =>
          resolve({ isSent: true, feedbackURL: response.url }),
        )
